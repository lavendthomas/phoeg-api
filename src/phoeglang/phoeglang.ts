...
const grammar: Grammar = {
  ...
  ParserRules: [
    ...
    {
      name: "disjonction$subexpression$1$string$1",
      symbols: [{ literal: "O" }, { literal: "R" }],
      postprocess: (d) => d.join(""),
    },
    {
      name: "disjonction$subexpression$1",
      symbols: ["disjonction$subexpression$1$string$1"],
    },
    {
      name: "disjonction$subexpression$1$string$2",
      symbols: [{ literal: "|" }, { literal: "|" }],
      postprocess: (d) => d.join(""),
    },
    {
      name: "disjonction$subexpression$1",
      symbols: ["disjonction$subexpression$1$string$2"],
    },
    {
      name: "disjonction",
      symbols: [
        "disjonction",
        "_",
        "disjonction$subexpression$1",
        "_",
        "exclusion",
      ],
      postprocess: (d) => d[0].merge(d[4], `(${d[0]}) OR (${d[4]})`),
    },
    { name: "exclusion", symbols: ["conjonction"], postprocess: id },
    {
      name: "exclusion$string$1",
      symbols: [{ literal: "X" }, { literal: "O" }, { literal: "R" }],
      postprocess: (d) => d.join(""),
    },
    {
      name: "exclusion",
      symbols: ["exclusion", "exclusion$string$1", "conjonction"],
      postprocess: (d) => d[0].merge(d[4], `(${d[0]}) XOR (${d[4]})`),
    },
    { name: "conjonction", symbols: ["negation"], postprocess: id },
    {
      name: "conjonction$subexpression$1$string$1",
      symbols: [{ literal: "A" }, { literal: "N" }, { literal: "D" }],
      postprocess: (d) => d.join(""),
    },
    {
      name: "conjonction$subexpression$1",
      symbols: ["conjonction$subexpression$1$string$1"],
    },
    {
      name: "conjonction$subexpression$1$string$2",
      symbols: [{ literal: "&" }, { literal: "&" }],
      postprocess: (d) => d.join(""),
    },
    {
      name: "conjonction$subexpression$1",
      symbols: ["conjonction$subexpression$1$string$2"],
    },
    {
      name: "conjonction",
      symbols: [
        "conjonction",
        "_",
        "conjonction$subexpression$1",
        "_",
        "negation",
      ],
      postprocess: (d) => d[0].merge(d[4], `(${d[0]}) AND (${d[4]})`),
    },
    { name: "negation", symbols: ["condition"], postprocess: id },
    {
      name: "negation$subexpression$1$string$1",
      symbols: [{ literal: "N" }, { literal: "O" }, { literal: "T" }],
      postprocess: (d) => d.join(""),
    },
    {
      name: "negation$subexpression$1",
      symbols: ["negation$subexpression$1$string$1"],
    },
    { name: "negation$subexpression$1", symbols: [{ literal: "!" }] },
    {
      name: "negation",
      symbols: ["negation$subexpression$1", "_", "condition"],
      postprocess: (d) => d[2].copy({ constraints: `NOT (${d[2]})` }),
    },
    {
      name: "negation$subexpression$2$string$1",
      symbols: [{ literal: "N" }, { literal: "O" }, { literal: "T" }],
      postprocess: (d) => d.join(""),
    },
    {
      name: "negation$subexpression$2",
      symbols: ["negation$subexpression$2$string$1"],
    },
    { name: "negation$subexpression$2", symbols: [{ literal: "!" }] },
    {
      name: "negation",
      symbols: [
        "negation$subexpression$2",
        "_",
        { literal: "(" },
        "_",
        "condition",
      ...
      postprocess: (d) => d[4].copy({ constraints: `NOT (${d[4]})` }),
    },
    { name: "condition", symbols: ["expression"], postprocess: id },
    {
      name: "condition",
      symbols: ["condition", "_", { literal: "<" }, "_", "expression"],
      postprocess: (d) => d[0].merge(d[4], `(${d[0]}) <  (${d[4]})`),
    },
    {
    ...
    {
      name: "condition",
      symbols: ["condition", "_", "condition$string$1", "_", "expression"],
      postprocess: (d) => d[0].merge(d[4], `(${d[0]}) <= (${d[4]})`),
    },
    {
      name: "condition",
    ...
    {
      name: "condition$string$2",
      symbols: [{ literal: ">" }, { literal: "=" }],
      postprocess: (d) => d.join(""),
    },
    {
      name: "condition",
      symbols: ["condition", "_", "condition$string$2", "_", "expression"],
      postprocess: (d) => d[0].merge(d[4], `(${d[0]}) >= (${d[4]})`),
    },
    { name: "condition$subexpression$1", symbols: [{ literal: "=" }] },
    {
      name: "condition$subexpression$1$string$1",
      symbols: [{ literal: "=" }, { literal: "=" }],
      postprocess: (d) => d.join(""),
    },
    {
      name: "condition$subexpression$1",
      symbols: ["condition$subexpression$1$string$1"],
    },
    {
      name: "condition",
      symbols: [
    ...
    {
      name: "condition$subexpression$2$string$1",
      symbols: [{ literal: "!" }, { literal: "=" }],
      postprocess: (d) => d.join(""),
    },
    {
      name: "condition$subexpression$2",
      symbols: ["condition$subexpression$2$string$1"],
    },
    {
      name: "condition$subexpression$2$string$2",
      symbols: [{ literal: "<" }, { literal: ">" }],
      postprocess: (d) => d.join(""),
    },
    {
      name: "condition$subexpression$2",
      symbols: ["condition$subexpression$2$string$2"],
    },
    {
      name: "condition",
      symbols: [
    ...
    {
      name: "parenthesed_expression",
      symbols: [{ literal: "(" }, "_", "expression", "_", { literal: ")" }],
      postprocess: (d) => d[2].copy({ constraints: `(${d[2]})` }),
    },
    ...
    {
      name: "expression",
      symbols: ["expression$string$1", "_", "parenthesed_term_list"],
      postprocess: (d) => d[2].copy({ constraints: `GREATEST ${d[2]}` }),
    },
    {
    ...
    {
      name: "expression",
      symbols: ["expression$string$2", "_", "parenthesed_term_list"],
      postprocess: (d) => d[2].copy({ constraints: `LEAST ${d[2]}` }),
    },
    {
    ...
    {
      name: "expression",
      symbols: ["expression$string$3", "_", "parenthesed_term_list"],
      postprocess: (d) => d[2].copy({ constraints: `SUM ${d[2]}` }),
    },
    {
    ...
    {
      name: "expression",
      symbols: ["expression$string$4", "_", "parenthesed_term_list"],
      postprocess: (d) => d[2].copy({ constraints: `AVG ${d[2]}` }),
    },
    {
    ...
    {
      name: "expression",
      symbols: ["expression$string$5", "_", "parenthesed_term_list"],
      postprocess: (d) => d[2].copy({ constraints: `MEAN ${d[2]}` }),
    },
    {
    ...
    {
      name: "expression",
      symbols: ["expression$string$6", "_", "parenthesed_term"],
      postprocess: (d) => d[2].copy({ constraints: `FLOOR ${d[2]}` }),
    },
    {
      name: "expression$string$7",
      symbols: [
        { literal: "C" },
        { literal: "E" },
        { literal: "I" },
    ...
    {
      name: "expression",
      symbols: ["expression$string$7", "_", "parenthesed_term"],
      postprocess: (d) => d[2].copy({ constraints: `CEIL ${d[2]}` }),
    },
    {
      name: "expression$string$8",
      symbols: [
        { literal: "R" },
        { literal: "O" },
        { literal: "U" },
    ...
    {
      name: "expression",
      symbols: ["expression$string$8", "_", "parenthesed_term"],
      postprocess: (d) => d[2].copy({ constraints: `ROUND ${d[2]}` }),
    },
    {
      name: "parenthesed_term",
      symbols: [{ literal: "(" }, "_", "term", "_", { literal: ")" }],
      postprocess: (d) => d[2].copy({ constraints: `(${d[2]})` }),
    },
    {
      name: "parenthesed_term_list",
      symbols: [{ literal: "(" }, "_", "term_list", "_", { literal: ")" }],
      postprocess: (d) => d[2].copy({ constraints: `(${d[2]})` }),
    },
    { name: "term_list", symbols: ["term"], postprocess: id },
    {
      name: "term_list",
      symbols: ["term_list", "_", { literal: "," }, "_", "term"],
      postprocess: (d) => d[0].merge(d[4], `${d[0]}, ${d[4]}`),
    },
    ...
    {
      name: "factor",
      symbols: ["factor", "_", { literal: "*" }, "_", "power"],
      postprocess: (d) => d[0].merge(d[4], `${d[0]} * ${d[4]}`),
    },
    {
      name: "factor",
    ...
    },
    {
      ...
      symbols: [{ literal: "m" }, { literal: "o" }, { literal: "d" }],
      postprocess: (d) => d.join(""),
    },
    {
      name: "factor",
      symbols: ["factor", "_", "factor$string$1", "_", "power"],
      postprocess: (d) => d[0].merge(d[4], `${d[0]} % ${d[4]}`),
    },
    { name: "factor", symbols: ["power"], postprocess: id },
    {
      name: "power",
      symbols: ["power", "_", { literal: "^" }, "_", "number"],
      postprocess: (d) => d[0].merge(d[4], `${d[0]} ^ ${d[4]}`),
    },
    { name: "power", symbols: ["number"], postprocess: id },
    {
      name: "number",
      symbols: ["decimal"],
      postprocess: (d) => PhoegLangResult.create({ constraints: `${d[0]}` }),
    },
    {
      name: "number",
      symbols: ["invariant"],
      postprocess: (d) =>
        PhoegLangResult.create({
          constraints: `${d[0]}.val`,
          invariants: [`${d[0]}`],
        }),
    },
    ...
    {
      name: "invariant$ebnf$1",
      symbols: ["invariant$ebnf$1", "invariant_char"],
      postprocess: (d) => d[0].concat([d[1]]),
    },
    {
      name: "invariant",
      symbols: ["invariant$ebnf$1"],
